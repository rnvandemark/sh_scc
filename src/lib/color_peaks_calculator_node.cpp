...
const std::vector<cv::Point2f> ColorPeaksCalculatorNode::WORLD_PLANE = {
    cv::Point2f(0,0),
    cv::Point2f(WORLD_WIDTH,0),
    cv::Point2f(0,WORLD_HEIGHT),
    cv::Point2f(WORLD_WIDTH,WORLD_HEIGHT)
};
const cv::Size ColorPeaksCalculatorNode::WORLD_SIZE = cv::Size(WORLD_WIDTH, WORLD_HEIGHT);
...
ColorPeaksCalculatorNode::ColorPeaksCalculatorNode() :
        ...
        window_right()
{
    screen_calibration_request_srv = create_service<sh_scc_interfaces::srv::RequestScreenCalibration>(
        ::sh::names::services::REQUEST_SCREEN_COLOR_CALIBRTION,
        std::bind(
            &sh::ColorPeaksCalculatorNode::screen_calibration_request_callback,
            this,
            std::placeholders::_1,
            std::placeholders::_2
        )
    );
    screen_calibration_set_homography_points_srv = create_service<sh_scc_interfaces::srv::SetScreenCalibrationPointsOfHomography>(
        ::sh::names::services::SET_SCREEN_COLOR_HOMOG_POINTS,
        std::bind(
            &sh::ColorPeaksCalculatorNode::screen_calibration_set_homography_points_callback,
            this,
            std::placeholders::_1,
            std::placeholders::_2
        )
...
void ColorPeaksCalculatorNode::screen_calibration_request_callback(
    ...
    sh_scc_interfaces::srv::RequestScreenCalibration::Response::SharedPtr res)
{
    ...
    for (std::vector<cv::Point2f>::iterator iter = corners.begin(); iter != corners.end(); ++iter)
    {
        ...
        cv::circle(cv_img_marked.image, cv::Point(x,y), cv_img_marked.image.cols/100, CV_RGB(0,255,0));
        sh_common_interfaces::msg::Point pt;
        pt.x = x;
        pt.y = y;
...
void ColorPeaksCalculatorNode::screen_calibration_set_homography_points_callback(
...
void ColorPeaksCalculatorNode::cap_image_raw_callback(const sensor_msgs::msg::Image::ConstSharedPtr msg)
{
    ...
    if (homog_set)
    {
        ...
        color_peaks_telem_msg.intermediate_images.push_back(*(last_received_frame->toImageMsg()));
        ...
        color_peaks_telem_msg.intermediate_images.push_back(*(cv_img_world.toImageMsg()));
